...
def git_clone_remote_repo(f: Callable):
    ...
    def wrapped_f(*args, **kwargs):
        ...
        if isinstance(kwargs["repo"], str) and _is_url_of_remote_repo(
            repo=kwargs["repo"]
        ):
            try:
                # noinspection PyTypeChecker
    ...
    def _is_url_of_remote_repo(repo: str) -> bool:
...
def cloned_git_repo(repo: str):
    ...
    _git_clone(repo=repo, dir=tmp_dir.name)
    yield tmp_dir.name
    logging.debug("delete temporary directory %s", tmp_dir)
    tmp_dir.cleanup()


def _git_clone(repo: str, dir: str) -> None:
